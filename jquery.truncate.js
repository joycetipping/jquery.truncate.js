...
//                                            var my_div = this.create ('div');
//                                            this.append (create ('div'));
//                                          }
...
//    else if (arg.constructor === Array)       result.create.apply (result, arg);
//    else if (arg.constructor === HTMLElement) result.append (arg);
//    else if (arg.constructor === HTMLAnchorElement) result.append (arg);
...
// create ('div').css ({margin:0, padding:2}).
//                text ('Hello world').
//                attr (

var create = function (first_arg) {
  var result = first_arg instanceof HTMLElement ? first_arg : document.createElement (first_arg);

  result.attr = function (obj) {
    for (var k in obj) {result[k] = obj[k];}
    return result;
  };
  result.set_class = function () {
    result.className = Array.prototype.slice.call (arguments).join (' ');
    return result;
  };
  result.add_class = function (str) {
    result.className += ' ' + Array.prototype.slice.call (arguments).join (' ');
    return result;
  }
  result.css = function (style) {
    if (style && style.constructor === Object) for (var k in style) result.style[k] = style[k];
    return result;
  };
  result.txt = function (str) {
    result.appendChild (result.text = document.createTextNode (str));
    return result;
  };
  result.append = function () {
    for (var i = 0, len = arguments.length; i < len; i++) {result.appendChild (arguments[i]);}
  ...
  result.prepend = function () {
    for (var i = 0, len = arguments.length; i < len; i++) {result.insertBefore (arguments[i], result.firstChild);}
    return result;
  };
  result.create = function () {
    return result.appendChild (create.apply (result, arguments));
  };
  ...
  for (var i = 1, len = arguments.length; i < len; i++) {
    var arg = arguments[i];
         if (arg.constructor === Object)   for (var k in arg) {result[k] = arg[k];}
    else if (arg.constructor === String)   result.txt (arg);
    else if (arg.constructor === Array)    result.create.apply (result, arg);
    else if (arg instanceof HTMLElement)   result.append (arg);
    else if (arg.constructor === Function) arg.call (result);
  }
  return result;
};
